...
// i assume that you want the /users instead of the /posts data for this function?
...
async function getComments (postID){
    const comments = await fetch('https://jsonplaceholder.typicode.com/posts/'+postID.toString()+'/comments')
...
async function getUser (userID){
    const user = await fetch('https://jsonplaceholder.typicode.com/users/'+userID.toString())
...
async function getTodosForUser(userID){
    ...
    isForUser = (obj) => obj.userId === userID;
    const userData = await data.filter(isForUser);
    return userData;
}